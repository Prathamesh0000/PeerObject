...
var Introspected = (O => {
  'use strict'; // commonly needed shortcuts
  ...
  const keys = O.keys;
  const toPrimitive = Symbol.toPrimitive; // weakly holds all known references
  // avoid useless Proxies when already known

  const known = new WeakMap(); // triggered on Array changes

  const ArrayHandler = ['copyWithin', 'fill', 'pop', 'push', 'reverse', 'shift', 'sort', 'splice', 'unshift'].reduce((properties, method) => {
    ...
    get(target, prop) {
      switch (true) {
        ...
        default:
          ...
          return target[prop] = register(info.O, create(null), info.$.concat(prop))._;
      }
    },
  ...
  function setValue(observer, target, path, prop, value) {
    if (isArray(value)) {
      target[prop] = (known.get(value) || importArray(observer, value, path.concat(prop)))._;
    } else if (typeof value === 'object' && value != null) {
      const object = getPrototypeOf(value) === SProto ? value.toJSON() : value;
      target[prop] = toString.call(object) === '[object Date]' ? object : (known.get(object) || importObject(observer, object, path.concat(prop)))._;
    } else {
      target[prop] = value;
    }
  }
  ...
  function wrap(target) {
    return isArray(target) ? defineProperties(target, ArrayHandler) : new Proxy(target, IntrospectedHandler);
  }

  function Introspected(target, callback) {
    const isNull = target == null;

    if (isNull || getPrototypeOf(target) !== SProto && !known.has(target)) {
      const root = isNull || !isArray(target) ? create(null) : [];
      const info = register(function (path) {
        const paths = this.$.concat(path);
        info.O.$.forEach(fn => fn(info._, paths));
      }, root, emptyArray);
      info.O.$ = new Set();
      if (!isNull) keys(target).forEach(prop => setValue(info.O, root, emptyArray, prop, target[prop]));
      ...
      if (callback) {
        known.get(target.toJSON()).O.$.add(callback); // don't notify other callbacks since no change was made
  ...
  Introspected.pathValue = (model, path) => path.reduce((m, p) => m && p in m ? m[p] : void 0, model);

  return Introspected;
})(Object);

try {
  module.exports = Introspected;
} catch (o_O) {}
/* TODO
  ...
  constructor(onError, options) {
      ...
      peer.on('connection', function (connection) {
        connection.on('open', function () {
          ...
          if (rejectedUsers.has(newUserID)) {
            rejectConnection(connection, ErrorType.PROHIBITED, `You\'ve been banned from the session "${sessionID}".`);
            return;
          ...
          if (newUserID === userID || connections.has(existingPeerName) || pending.has(existingPeerName)) {
            rejectConnection(connection, ErrorType.DUPLICATE_USER_ID, `User ID "${newUserID}" is already taken.`);
            return;
          }

          var peerName = connection.peer; //An existing peer might change it's user ID.
    ...
    */
    ...
    this.acceptUser = function (newUserID) {
      ...
      connection.on('data', dataReceived);
      connection.on('close', connectionClosed);
      connection.send(makeMessage(MsgType.PEER_LIST, Array.from(connections.keys())));
      sendIdentity(connection);
      var event = createEvent('userpresent', {
        sessionID: sessionID,
        userID: newUserID,
